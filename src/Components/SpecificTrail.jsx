...
class SpecificTrail extends Component {
  ...
  render() {
    ...
    if (trail) {
        ...
        <>
          ...
          <Container textAlign="justified" id="specific-trail">
            <Grid columns={2}>
              <Grid.Row>
                <Grid.Column width={5}>
                  <Image id={`image_${trail.id}`} src={trail.image} />
                </Grid.Column>
                <Grid.Column>
                  ...
                  <p
                    ...
                    id={`description_${trail.id}`}
                  >
                    {trail.description}
                  </p>
                  <div className="single-trail">
                    <h3>Good to know:</h3>
                    <p className="single-content" id={`extra_${trail.id}`}>
                      {trail.extra}
                    </p>
                  </div>
                  <div className='single-trail'>
                  ...
                  <div className="single-trail">
                    <h3>Continent:</h3>
                    <p className="single-content" id={`continent_${trail.id}`}>
                      {trail.continent}
                    </p>
                  </div>
                  <div className="single-trail">
                    <h3>Duration:</h3>
                    <p className="single-content" id={`duration_${trail.id}`}>
                      {trail.duration} min
                    </p>
                  </div>
                  <div className="single-trail">
                    <h3>Intensity Level:</h3>
                    <p className="single-content" id={`intensity_${trail.id}`}>
                      {trail.intensity}
                    </p>
                  </div>
                </Grid.Column>
      ...
      singleTrail = <h3 id="error-message">{this.state.errorMessage}</h3>;
    }

    backButton = (
      <a id="back-button" onClick={this.goBack} href="#">
        Go Back
      </a>
    );
